...
function createIdsList() {
  for (let i = 0; i < lines.length; i++) {
    ...
    const pp = line.split('').reduce(handleCharCodes(), {
      row: { start: 0, end: 127 },
    ...
    const seatId = pp.seatId(pp.row.start, pp.column.start);
    ids.push(seatId);
  }
}

function handleCharCodes() {
  // should refactor to reduce redundancy within conditions
  return (acc, x) => {
    if (x === 'F') {
